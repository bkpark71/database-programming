...
public class MainApp {
  ...
  private static void addCustomer() throws SQLException {
    ...
    custinfos = in.next().split(",");
    sql = "insert into customer values (?,?,?,?);";
    pstmt = conn.prepareStatement(sql);
    pstmt.setInt(1,custid + 1);
    pstmt.setString(2,custinfos[0]);
    pstmt.setString(3,custinfos[1]);
    pstmt.setString(4,custinfos[2]);
    res = pstmt.executeUpdate();
    if(res == 1) System.out.println("등록완료!");
  }

  private static void addOrder() throws SQLException {
    Integer res = 0 , custId = 0, bookId = 0, bookPrice = 0, salePrice = 0;
    String[] orderinfos;
    String sql="";
    String name ="", bookInfo = "";

    System.out.print("주문을 등록할 고객id,도서id를 입력하세요.");
    orderinfos = in.next().split(",");
